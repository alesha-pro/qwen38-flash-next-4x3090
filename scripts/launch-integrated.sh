...
MODEL_DIR="${MODEL_DIR:-/mnt/ssd/models/Qwen3.8-Flash-Next-W4A16}"
...
PORT="${PORT:-8018}"
HOST="${HOST:-127.0.0.1}"
CONTAINER_NAME="${CONTAINER_NAME:-qwen38-w4a16-fp8kv}"
...
if [[ -z "${COMPILATION_CONFIG:-}" ]]; then
    ...
    COMPILATION_CONFIG='{"mode":"VLLM_COMPILE","cudagraph_mode":"FULL_AND_PIECEWISE"}'
fi
DIST=/usr/local/lib/python3.12/dist-packages
VLLM_PKG="${DIST}/vllm"

[[ -f "${MODEL_DIR}/model.safetensors.index.json" ]] || {
    echo "W4A16 checkpoint missing" >&2; exit 2; }

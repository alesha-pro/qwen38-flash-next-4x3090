...
# SPLIT MODE: no HC weight mapper — separate modules load directly
...
class Qwen3_8FlashNextModel(nn.Module):
    # SPLIT MODE: use Qwen3NextModel mapper directly, skipping Qwen3_5's
    # fused in_proj_qkvz / in_proj_ba mappings (we split them at runtime).
    hf_to_vllm_mapper = Qwen3NextModel.hf_to_vllm_mapper | _HC_WEIGHTS_MAPPER
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        skip_substrs = [
            ...
            "shared_expert_gate.weight_scale",
        ]
...
class Qwen3_8FlashNextForConditionalGeneration(
    ...
    packed_modules_mapping = {
        "qkv_proj": ["q_proj", "k_proj", "v_proj"],
        "gate_up_proj": ["gate_proj", "up_proj"],
        # in_proj_qkv: single fused checkpoint tensor, MergedColumnParallelLinear
        # splits it by output_sizes. in_proj_z/b/a are separate.
    }
    # SPLIT MODE: same prefix mapping as vendor Qwen3VLForConditionalGeneration
    # (visual/lm_head/language_model), but no fused GDN/HC stacked mappings.
    hf_to_vllm_mapper = WeightsMapper(
        orig_to_new_prefix={
            "model.visual.": "visual.",

...
class PleOffloadRunner:
    ...
    def _load_weights(self) -> None:
        ...
        # qwen38-quantize Phase 1: optionally stream PLE weights from an
        # explicit secondary checkpoint (e.g. RadixArk FP8 PLE) while the
        # primary model (W4A16) keeps BF16 PLE shards unused. Fail-closed:
        # requires VLLM_PLE_MANIFEST and validates revision, tensor set,
        # scale value and payload hashes before any weight is read.
        weight_source_config = model_config
        if os.environ.get("VLLM_PLE_MODEL_PATH", "").strip():
            from vllm.v1.ple_offload.ple_external_source import (
                resolve_external_ple_model_config,
            )
            weight_source_config = resolve_external_ple_model_config(
        ...
        if isinstance(loader, DummyModelLoader):
            ...
            all_weights = loader.get_all_weights(weight_source_config, model)

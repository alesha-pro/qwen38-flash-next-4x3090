...
def _load_external_ple_scale() -> torch.Tensor:
    """Load the global FP8 PLE scale from the external PLE checkpoint.

    Fail-closed: validates the payload against the manifest (sha256 + value)
    before returning. Used on GPU workers when the primary checkpoint has no
    PLE scale in its stream (W4A16 + external FP8 PLE composition).
    """
    import hashlib
    import json
    import struct

    root = os.environ["VLLM_PLE_MODEL_PATH"].strip()
    manifest_path = os.environ.get("VLLM_PLE_MANIFEST", "").strip()
    if not manifest_path:
        raise RuntimeError(
            "VLLM_PLE_MODEL_PATH requires VLLM_PLE_MANIFEST for the PLE scale"
        )
    with open(manifest_path) as f:
        manifest = json.load(f)
    scale_meta = manifest.get("scale") or {}
    name = scale_meta.get("name")
    if not name:
        raise RuntimeError("PLE manifest has no scale entry")
    ...
    with open(shard_path, "rb") as f:
        ...
        entry = header[name]
        if entry["dtype"] != "BF16" or entry["shape"] != [1]:
            raise RuntimeError(
                f"external PLE scale has unexpected layout: {entry}"
            )
        a, b = entry["data_offsets"]
        f.seek(8 + hlen + a)
        raw = f.read(b - a)
    if hashlib.sha256(raw).hexdigest() != scale_meta.get("sha256"):
        raise RuntimeError("external PLE scale payload hash mismatch")
    bits = struct.unpack("<H", raw)[0]
    value = struct.unpack("<f", struct.pack("<I", bits << 16))[0]
    if value != scale_meta.get("value"):
        raise RuntimeError("external PLE scale value mismatch")
...
class Qwen3_8FlashNextNGramEmbedding(PleOffloadLayer):
    def __init__(
        ...
        ple_quant_method = _get_ple_embedding_quant_method(
            quant_config, f"{prefix}.ngram_embedding"
        )
        # Qwen3.8-Flash-Next ModelOpt checkpoints quantize the main model as
        # NVFP4 but store PLE in separate FP8 shards.  The main ModelOpt config
        # is not an Fp8Config, so infer the PLE method from its explicit dtype
        # instead.  Without this, the CPU offload worker allocates a BF16 table
        # (roughly doubling host RAM) and has no weight_scale parameter for the
        # checkpoint's global FP8 scale.
        ple_embedding_dtype = str(
            getattr(config, "ple_embedding_dtype", "")
        ).removeprefix("torch.")
        # qwen38-quantize Phase 1: when the PLE table is sourced from an external
        # FP8 checkpoint (VLLM_PLE_MODEL_PATH), the primary W4A16 config has
        # no ple_embedding_dtype key; allow an explicit env override so the
        # FP8 embedding method (FP8 storage + global weight_scale) is selected
        # for the CPU-offloaded table.
        if not ple_embedding_dtype and os.environ.get(
                "VLLM_PLE_MODEL_PATH", "").strip():
            ple_embedding_dtype = os.environ.get(
                "VLLM_PLE_EMBEDDING_DTYPE", "").strip()
        if ple_quant_method is None and ple_embedding_dtype == "float8_e4m3fn":
            ple_quant_method = Qwen3_8FlashNextPLEFp8EmbeddingMethod()
    ...
    def get_offload_output_dtype(self, default_dtype: torch.dtype) -> torch.dtype:
        ...
        if os.environ.get("QWEN38_DEBUG_PLE") == "1" and not getattr(
            self, "_ple_dtype_logged", False
        ):
            self._ple_dtype_logged = True
            import sys as _sys
            chosen = (weight.dtype if weight is not None
                      else (torch.float8_e4m3fn
                            if hasattr(self, "_offload_weight_scale")
                            else default_dtype))
            print(
                "QWEN38_DEBUG_PLE offload_output_dtype:"
                f" has_weight={weight is not None}"
                f" weight_dtype={None if weight is None else weight.dtype}"
                f" has_offload_scale={hasattr(self, '_offload_weight_scale')}"
                f" chosen={chosen}",
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        if envs.VLLM_PLE_CPU_OFFLOAD and not is_offload_process():
            ...
            for name, loaded_weight in weights:
                ...
                retained.add(name)
            # qwen38-quantize: with an external FP8 PLE source the primary
            # checkpoint (W4A16) has no ngram_embedding.weight_scale in its
            # stream, leaving the GPU side without the global scale: the
            # offload transport then falls back to BF16 and the FP8 lookup is
            # never dequantized (observed 2026-08-27 as ~300x PLE magnitude
            # and token soup). Load the 2-byte scale from the external
            # checkpoint, fail-closed against the manifest.
            if not hasattr(self, "_offload_weight_scale") and os.environ.get(
                "VLLM_PLE_MODEL_PATH", ""
            ).strip():
                scale = _load_external_ple_scale()
                self.register_buffer(
                    "_offload_weight_scale",
                    scale.to(device=torch.accelerator.current_accelerator()),
                    persistent=False,
                )
                retained.add("ngram_embedding.weight_scale")
            return retained
...
class Qwen3_8FlashNextPLELayer(nn.Module, MambaBase):
    ...
    def _dequantize_embeddings(
        ...
        if os.environ.get("QWEN38_DEBUG_PLE") == "1" and not getattr(
            self, "_ple_debug_logged", False
        ):
            self._ple_debug_logged = True
            inner = getattr(self.ple_embedding, "ngram_embedding", None)
            inner_scale = getattr(inner, "weight_scale", None)
            offload_scale = getattr(self.ple_embedding, "_offload_weight_scale", None)
            import sys as _sys
            print(
                "QWEN38_DEBUG_PLE dequantize:"
                f" emb_dtype={embeddings.dtype}"
                f" emb_abs_mean={embeddings.float().abs().mean().item():.6f}"
                f" inner_scale={None if inner_scale is None else float(inner_scale.float().mean())}"
                f" offload_scale={None if offload_scale is None else float(offload_scale.float().mean())}"
                f" used_scale={None if weight_scale is None else float(weight_scale.float().mean())}",
                file=_sys.stderr, flush=True,
            )

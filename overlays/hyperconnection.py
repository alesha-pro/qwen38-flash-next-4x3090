...
class GatedResidual(nn.Module):
    ...
    def __init__(
        ...
        # -- vLLM Linear weights --------------------------------------------
        # SPLIT MODE (VLLM_HYPER_SPLIT_PROJ=1): separate ReplicatedLinear for
        # input_mix_weight_down (FP8) and block_inject_weight (BF16), avoiding
        # the MergedColumnParallelLinear that cannot hold mixed quant schemes.
        import os as _os
        self.pad_size = (-(self.lora_rank + self.hc_count)) % 16 if use_combine else 0
        if use_combine and _os.environ.get("VLLM_HYPER_SPLIT_PROJ") == "1":
            _qcfg = quant_config
            ...
            self.block_inject_weight = ReplicatedLinear(
                ...
                bias=False,
                params_dtype=config.params_dtype,
                quant_config=None,  # BF16 — not in FP8 targets
            ...
            self.input_mix_weight_down = ReplicatedLinear(
                ...
                quant_config=(quant_config
                              if _os.environ.get("VLLM_HYPER_SPLIT_PROJ") == "1"
                              else None),
                prefix=maybe_prefix(prefix, "input_mix_weight_down"),
                return_bias=False,
            )
        _up_qcfg = (quant_config
                    if _os.environ.get("VLLM_HYPER_SPLIT_PROJ") == "1"
                    else None)
    ...
    def mix(
        ...
        if self.use_combine and hasattr(self, "input_mix_weight_down_block_inject"):
        ...
        elif self.use_combine:
            # SPLIT MODE: separate down and block_inject projections

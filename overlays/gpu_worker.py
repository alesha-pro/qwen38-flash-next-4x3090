...
class Worker(WorkerBase):
    def __init__(
        ...
        self._ple_offload_config_payload: bytes | None = None
        if envs.VLLM_PLE_CPU_OFFLOAD:
            if self._ple_offload_enabled:
                self._validate_ple_offload_config()
                # Model construction attaches local weight-loader closures to
                # the live config, making it unpicklable.  The PLE process is
                # deliberately spawned after model loading to keep its CUDA
                # context out of the Marlin repack peak, so preserve a clean
                # serialized snapshot while the config is still picklable.
                if self.rank == 0 and self.parallel_config.data_parallel_rank == 0:
                    self._ple_offload_config_payload = pickle.dumps(vllm_config)
    ...
    def spawn_ple_offload(self) -> None:
        ...
        if self._ple_offload_config_payload is None:
            raise RuntimeError("PLE offload config snapshot was not prepared")
        ple_vllm_config = pickle.loads(self._ple_offload_config_payload)
    ...
    def compile_or_warm_up_model(self) -> CompilationTimes:
        ...
        if self.use_v2_model_runner:
            ...
            # warmup_kernels drives several synthetic SchedulerOutputs back to back.
            # PLE offload normally stages inputs on a background D2H stream, but the
            # synthetic loop can re-record its single input-ready CUDA event before
            # that thread has consumed the previous request.  The D2H stream then
            # waits on a later event recorded behind the model's PLE semaphore wait,
            # creating a startup deadlock.  PLE values are irrelevant for kernel
            # compilation, so satisfy its placeholders locally during this warmup,
            # just as ModelRunner's ordinary dummy runs already do.
            ple_connector = self.model_runner._ple_offload_connector
            if ple_connector is None:
                warmup_kernels(self.model_runner, self.execute_model, self.sample_tokens)
            else:
                original_prepare_forward = ple_connector.prepare_forward

                def prepare_ple_dummy_forward(
                    num_reqs: int, num_tokens: int, dummy_run: bool
                ) -> None:
                    del num_reqs, dummy_run
                    ple_connector.signal_dummy_outputs(num_tokens)

                logger.info(
                    "Using local dummy PLE outputs during V2 synthetic kernel warmup."
                )
                ple_connector.prepare_forward = prepare_ple_dummy_forward
                try:
                    warmup_kernels(
                        self.model_runner, self.execute_model, self.sample_tokens
                    )
                finally:
                    ple_connector.prepare_forward = original_prepare_forward

...
class WorkerProc:
    ...
    def __init__(
        ...
        if envs.VLLM_PLE_CPU_OFFLOAD:
            # The PLE process eventually imports CUDA IPC buffers, but its
            # model-discovery path can initialize a ~322 MiB CUDA context on
            # GPU 0 long before registration.  On 24 GiB cards that steals the
            # last few MiB needed by NVFP4-to-Marlin conversion.  GPU workers
            # can safely connect their ZMQ PUSH sockets before the PLE worker
            # binds; ZeroMQ queues the registration payloads.  Spawn only after
            # every local worker has completed model loading so the transient
            # Marlin repack peak is gone before that CUDA context appears.
